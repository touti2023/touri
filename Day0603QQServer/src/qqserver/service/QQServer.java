...
public class QQServer {
    ...
    //验证用户是否有效的方法
    private boolean chenckUser(String userId,String passwd) {
    ...
    public QQServer() {
        ...
        try {
            ...
            while (true) {//即使和某个客户端连接后，仍然会继续监听。循环
                ...
                //验证
                if (chenckUser(u.getUserId(),u.getPasswd())) {//合法
            ...
            //如果服务器退出了while，说明服务器不再监听，关闭ServerSocket
            try {
                ss.close();
            } catch (IOException e) {
                e.printStackTrace();
